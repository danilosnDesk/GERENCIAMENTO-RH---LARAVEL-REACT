import { MoveRight } from "lucide-react";
import { useState } from "react";
import { TEInput } from "tw-elements-react";

export const CompleteForm = ({ funcionario }) => {
    const [Loading, setLoading] = useState(false);
    const [ProfileImg, setProfileImg] = useState(null);
    const [ImgError, setImagError] = useState(null);

    const setProfileIMG = (event) => {
        const uploadedImage = event.target.files[0];
        if (uploadedImage) {

            if (uploadedImage.type === 'image/png' || uploadedImage.type === 'image/jpeg' || uploadedImage.type === 'image/jpg') {
                const imageURL = URL.createObjectURL(uploadedImage);
                setProfileImg(imageURL);
                console.log(uploadedImage);
                setImagError(null);
            } else {
                setImagError('Formato não suportado..!');
            }

        }
    }
    const onSubimt = (ev) => {
        ev.preventDefault();
        alert('ok')
    }
    return (
        <div className="p-4 ">
            <form className="py-4 ">
                <div className="flex items-center gap-20 py-4 bg-slate-100 dark:bg-gray-800 dark:text-slate-400">
                    <div className="flex w-full flex-col gap-4">
                        <img className="w-[150px] h-[150px] rounded-2xl" src="data:image/jpeg;base64," alt="" />
                        <input type="file" className="file:bg-violet-700 file:border-none file:text-white " onChange={setProfileIMG} />
                    </div>
                    {ImgError && <div><span className="text-slate-400">{ImgError}</span></div>}

                    {!ImgError && ProfileImg && <MoveRight className="w-[40px] text-slate-400" />}
                    {!ImgError && ProfileImg &&
                        <div className="flex w-full flex-col gap-4">
                            <img
                                className="w-[150px] h-[150px] rounded-2xl"
                                src={ProfileImg}
                                accept=".png"
                            />
                        </div>
                    }


                </div>

                <div className="pt-4">
                    <h5 className="text-[18px] text-primary dark:text-slate-300">Dados Profissionais</h5>

                    <div className="flex items-center justify-between  py-2">
                        <span className="text-[16px] text-[#2c384e] w-1/2 dark:text-slate-500">Nome Completo</span>
                        <input type="text" value={funcionario.nome} placeholder="Nome completo" className="dark:bg-gray-700 dark:border-slate-500 w-full py-1 px-1 text-gray-800 outline-none  border-2 border-slate-400 rounded focus:border-violet-700" />
                    </div>
                    <div className="flex items-center justify-between  py-2">
                        <span className="text-[16px] text-[#2c384e] w-1/2 dark:text-slate-500">Nacionalidade</span>
                        <input type="text" value="Angolana" placeholder="Cargo" className="dark:bg-gray-700 dark:border-slate-500 w-full py-1 px-1 text-gray-800 outline-none  border-2 border-slate-400 rounded focus:border-violet-700" />
                    </div>
                    <div className="flex items-center justify-between  py-2">
                        <span className="text-[16px] text-[#2c384e] w-1/2 dark:text-slate-500">Nascimento</span>
                        <input type="text" value="21/09/2***" placeholder="Cargo" className="dark:bg-gray-700 dark:border-slate-500 w-full py-1 px-1 text-gray-800 outline-none  border-2 border-slate-400 rounded focus:border-violet-700" />
                    </div>
                </div>



                <div className="pt-4">
                    <h5 className="text-[18px] text-primary dark:text-slate-300">Dados Profissionais</h5>
                    <div className="flex items-center justify-between  py-2">
                        <span className="text-[16px] text-[#2c384e] w-1/2 dark:text-slate-500">Departamento</span>
                        <select name="" id="" className="dark:bg-gray-700 dark:border-slate-500 w-full py-1 px-1 text-gray-800 outline-none  border-2 border-slate-400 rounded focus:border-violet-700">
                            <option value="">Selecione o Departamento</option>
                            <option selected value={funcionario.departamento.id}>{funcionario.departamento.nome}</option>
                        </select>
                    </div>
                    <div className="flex items-center justify-between  py-2">
                        <span className="text-[16px] text-[#2c384e] w-1/2 dark:text-slate-500">Cargo</span>
                        <input value={funcionario.cargo} type="text" placeholder="Cargo" className="dark:bg-gray-700 dark:border-slate-500 w-full py-1 px-1 text-gray-800 outline-none  border-2 border-slate-400 rounded focus:border-violet-700" />
                    </div>
                    <div className="flex items-center justify-between  py-2">
                        <span className="text-[16px] text-[#2c384e] w-1/2 dark:text-slate-500">Data de entrada</span>
                        <input value={funcionario.data_entrada} type="text" placeholder="Cargo" className="dark:bg-gray-700 dark:border-slate-500 w-full py-1 px-1 text-gray-800 outline-none  border-2 border-slate-400 rounded focus:border-violet-700" />
                    </div>

                </div>

                <div className="pt-4">
                    <h5 className="text-[18px] text-primary dark:text-slate-300">Seus contactos</h5>
                    <div className="flex items-center justify-between  py-2">
                        <span className="text-[16px] text-[#2c384e] w-1/2 dark:text-slate-500">Email</span>
                        <input value={funcionario.email} type="text" placeholder="Nome completo" className="dark:bg-gray-700 dark:border-slate-500 w-full py-1 px-1 text-gray-800 outline-none  border-2 border-slate-400 rounded focus:border-violet-700" />
                    </div>
                    <div className="flex items-center justify-between  py-2">
                        <span className="text-[16px] text-[#2c384e] w-1/2 dark:text-slate-500">Telefone</span>
                        <input value={funcionario.telefone} type="text" placeholder="Cargo" className="dark:bg-gray-700 dark:border-slate-500 w-full py-1 px-1 text-gray-800 outline-none  border-2 border-slate-400 rounded focus:border-violet-700" />
                    </div>
                    <div className="flex items-center justify-between  py-2">
                        <span className="text-[16px] text-[#2c384e] w-1/2 dark:text-slate-500">LinkedIn</span>
                        <input value={funcionario.nome} type="text" placeholder="Cargo" className="dark:bg-gray-700 dark:border-slate-500 w-full py-1 px-1 text-gray-800 outline-none  border-2 border-slate-400 rounded focus:border-violet-700" />
                    </div>

                </div>
                <button onClick={onSubimt} className="rounded shadow text-white bg-violet-700 mt-2 py-2 px-4 w-[170px]">
                    {Loading ? 'carregendo...' : 'Salvar alterações'}
                </button>
            </form>
        </div>
    )
}
